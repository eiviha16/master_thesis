...
class QTM:
    ...
    def temporal_difference(self, next_q_vals):
        return np.array(self.replay_buffer.sampled_rewards) + (
                1 - np.array(self.replay_buffer.sampled_terminated)) * self.config["gamma"] * next_q_vals
    ...
    def get_q_val_and_obs_for_tm(self, actions, target_q_vals, cur_obs):
        tm_inputs = [{'observations': [], 'target_q_vals': []} for _ in range(self.action_space_size)]
        for index, action in enumerate(actions):
            tm_inputs[action[0]]['observations'].append(cur_obs[index][0])
    ...
    def learn(self, nr_of_episodes):
        for episode in tqdm(range(nr_of_episodes)):
            ...
            #    break
            self.rollout()
            if self.total_timesteps >= self.config["sample_size"] + self.config["n_steps"]:
...
class DoubleQTM(QTM):
    ...
    def train(self):
        for i in range(self.config["sampling_iterations"]):
            self.replay_buffer.sample()
            actions = np.argmax(self.online_policy.predict(np.array(self.replay_buffer.sampled_next_obs)), axis=1)

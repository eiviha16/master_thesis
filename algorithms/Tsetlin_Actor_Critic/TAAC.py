...
#### Tsetlin Advantage Actor Critic ####
...
class TAAC:
    ...
    def get_update_data_actor(self):
        tm = {'observations': [], 'actions': [], 'advantages': []}
        for i in range(len(self.batch.actions)):
            tm['observations'].append(self.batch.obs[i])
            tm['actions'].append(self.batch.actions[i])
            tm['advantages'].append(self.batch.advantages[i])
        return tm
    ...
    def get_update_data_critic(self):
        ...
        for i in range(len(self.batch.actions)):
            tm['observations'].append(self.batch.obs[i])
            tm['target'].append(self.batch.advantages[i] + self.batch.values[i, 0])
        return tm

    def train(self):
        for _ in range(self.epochs):
            actor_update = self.get_update_data_actor()
            self.policy.actor.update(actor_update)

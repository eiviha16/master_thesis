...
cdef class TsetlinMachine:
	...
	cpdef void update_2(self, int[:] X, float y, float advantage):
			cdef int i, j
			cdef int action_include, action_include_negated
			cdef float output_sum
			cdef float output_value
	...
	def fit_2(self, int[:,:] X, float[:] y, float[:] advantages, int epochs=1):
		cdef int j, l, epoch
		cdef int example_id
		cdef float target_class
		cdef int[:] Xi
		...
		for epoch in xrange(epochs):
			...
			for l in xrange(number_of_examples):
				...
				for j in xrange(self.number_of_features):
					Xi[j] = X[example_id,j]
				self.update_2(Xi, target_class, advantage)

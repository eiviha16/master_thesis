...
def objective(config):
    ...
    _config = {'comment': '3/20/2024', 'algorithm': 'TM_DDPG_2', 'soft_update_type': 'soft_update_2', 'exploration_prob_init': config.exploration_p_init, 'exploration_prob_decay': config.exploration_p_decay, 'update_grad': 0, "update_freq": config.update_freq, 'gamma': config.gamma,
               "buffer_size": config.buffer_size, 'actor': actor, 'critic': critic, 'batch_size': config.batch_size, 'epochs': config.epochs, 'test_freq': 1, "save": True, "dataset_file_name": "observation_data"}
...
def main():
    wandb.init(project="TAC_b_cartpole")
...
sweep_id = wandb.sweep(sweep=sweep_configuration, project="TAC_b_cartpole")

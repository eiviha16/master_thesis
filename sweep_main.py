...
def start_c_TAC_a():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_TAC_a, project="cartpole-TAC_a")
    wandb.agent(sweep_id, function=main_c_TAC_a(), count=10_000)

def start_a_TAC_a():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_TAC_a, project="acrobot-TAC_a")
...
################################################
def main_c_TAC_b():
    wandb.init(project="cartpole-TAC_b")
    score = cartpole_TAC_b(wandb.config)
    wandb.log({"score": score})
def main_a_TAC_b():
    wandb.init(project="acrobot-TAC_b")
...
def start_c_TAC_b():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_TAC_b, project="cartpole-TAC_b")
    wandb.agent(sweep_id, function=main_c_TAC_b(), count=10_000)


def start_a_TAC_b():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_TAC_b, project="acrobot-TAC_b")
...
def start_c_TPPO():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_TPPO, project="cartpole-TPPO")
    wandb.agent(sweep_id, function=main_c_TPPO(), count=10_000)

def start_a_TPPO():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_TPPO, project="acrobot-TPPO")
...
def main_c_n_step_DQTM_a():
    wandb.init(project="cartpole-n_step_DQTM_a")
    score = cartpole_n_step_DQTM_a(wandb.config)
    wandb.log({"score": score})
def main_a_n_step_DQTM_a():
    wandb.init(project="acrobot-n_step_DQTM_a")
    score = acrobot_n_step_DQTM_a(wandb.config)
    wandb.log({"score": score})

def start_c_n_step_DQTM_a():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_n_step_DQTM_a, project="cartpole-n_step_DQTM_a")
    wandb.agent(sweep_id, function=main_c_n_step_DQTM_a(), count=10_000)

def start_a_n_step_DQTM_a():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_n_step_DQTM_a, project="acrobot-n_step_DQTM_a")
...
def start_c_n_step_DQTM_b():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_n_step_DQTM_b, project="cartpole-n_step_DQTM_b")
    wandb.agent(sweep_id, function=main_c_n_step_DQTM_b(), count=10_000)

def start_a_n_step_DQTM_b():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_n_step_DQTM_b, project="acrobot-n_step_DQTM_b")
    wandb.agent(sweep_id, function=main_a_n_step_DQTM_b(), count=10_000)
################################################
############ Double TMQN type a ################
################################################
def main_c_DQTM_a():
    wandb.init(project="cartpole-DQTM_a")
    score = cartpole_DQTM_a(wandb.config)
    wandb.log({"score": score})
def main_a_DQTM_a():
    wandb.init(project="acrobot-DQTM_a")
    score = acrobot_DQTM_a(wandb.config)
    wandb.log({"score": score})
def start_c_DQTM_a():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_DQTM_a, project="cartpole-DQTM_a")
    wandb.agent(sweep_id, function=main_c_DQTM_a(), count=10_000)

def start_a_DQTM_a():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_DQTM_a, project="acrobot-DQTM_a")
    wandb.agent(sweep_id, function=main_a_DQTM_a(), count=10_000)
################################################
############ Double TMQN type b ################
################################################
def main_c_DQTM_b():
    wandb.init(project="cartpole-DQTM_b")
    score = cartpole_DQTM_b(wandb.config)
    wandb.log({"score": score})
def main_a_DQTM_b():
    wandb.init(project="acrobot-DQTM_b")
    score = acrobot_DQTM_b(wandb.config)
    wandb.log({"score": score})
def start_c_DQTM_b():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_DQTM_b, project="cartpole-DQTM_b")
    wandb.agent(sweep_id, function=main_c_DQTM_b(), count=10_000)

def start_a_DQTM_b():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_DQTM_b, project="acrobot-DQTM_b")
    wandb.agent(sweep_id, function=main_a_DQTM_b(), count=10_000)
################################################
################# n-step TMQN  #################
################################################
def main_c_n_step_QTM():
    wandb.init(project="cartpole-n_step_QTM")
    score = cartpole_n_step_QTM(wandb.config)
    wandb.log({"score": score})
def main_a_n_step_QTM():
    wandb.init(project="acrobot-n_step_QTM")
    score = acrobot_n_step_QTM(wandb.config)
    wandb.log({"score": score})
def start_c_n_step_QTM():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_n_step_QTM, project="cartpole-n_step_QTM")
    wandb.agent(sweep_id, function=main_c_n_step_QTM(), count=10_000)

def start_a_n_step_QTM():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_n_step_QTM, project="acrobot-n_step_QTM")
...
def start_c_QTM():
    import wandb
    sweep_id = wandb.sweep(sweep=cartpole_QTM, project="cartpole-QTM")
    wandb.agent(sweep_id, function=main_c_QTM(), count=10_000)

def start_a_QTM():
    import wandb
    sweep_id = wandb.sweep(sweep=acrobot_QTM, project="acrobot-QTM")

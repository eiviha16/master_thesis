...
def start_c_TAC_a():
    import wandb
    sweep_id = wandb.sweep(sweep=config_cartpole_TAC_a, project="cartpole-TAC_a")
    wandb.agent(sweep_id, function=main_c_TAC_a, count=10_000)

def start_a_TAC_a():
    import wandb
    sweep_id = wandb.sweep(sweep=config_acrobot_TAC_a, project="acrobot-TAC_a")
...
def start_c_TAC_b():
    import wandb
    sweep_id = wandb.sweep(sweep=config_cartpole_TAC_b, project="cartpole-TAC_b")
    wandb.agent(sweep_id, function=main_c_TAC_b, count=10_000)


def start_a_TAC_b():
    import wandb
    sweep_id = wandb.sweep(sweep=config_acrobot_TAC_b, project="acrobot-TAC_b")
...
def start_c_n_step_DQTM_a():
    import wandb
    sweep_id = wandb.sweep(sweep=config_cartpole_n_step_DQTM_a, project="cartpole-n_step_DQTM_a")
    wandb.agent(sweep_id, function=main_c_n_step_DQTM_a, count=10_000)

def start_a_n_step_DQTM_a():
    import wandb
    sweep_id = wandb.sweep(sweep=config_acrobot_n_step_DQTM_a, project="acrobot-n_step_DQTM_a")
...
def start_c_n_step_DQTM_b():
    import wandb
    sweep_id = wandb.sweep(sweep=config_cartpole_n_step_DQTM_b, project="cartpole-n_step_DQTM_b")
    wandb.agent(sweep_id, function=main_c_n_step_DQTM_b, count=10_000)

def start_a_n_step_DQTM_b():
    import wandb
    sweep_id = wandb.sweep(sweep=config_acrobot_n_step_DQTM_b, project="acrobot-n_step_DQTM_b")
...
def start_c_DQTM_a():
    import wandb
    sweep_id = wandb.sweep(sweep=config_cartpole_DQTM_a, project="cartpole-DQTM_a")
    wandb.agent(sweep_id, function=main_c_DQTM_a, count=10_000)

def start_a_DQTM_a():
    import wandb
    sweep_id = wandb.sweep(sweep=config_acrobot_DQTM_a, project="acrobot-DQTM_a")
...
def start_c_DQTM_b():
    import wandb
    sweep_id = wandb.sweep(sweep=config_cartpole_DQTM_b, project="cartpole-DQTM_b")
    wandb.agent(sweep_id, function=main_c_DQTM_b, count=10_000)

def start_a_DQTM_b():
    import wandb
    sweep_id = wandb.sweep(sweep=config_acrobot_DQTM_b, project="acrobot-DQTM_b")
...
def start_c_n_step_QTM():
    import wandb
    sweep_id = wandb.sweep(sweep=config_cartpole_n_step_QTM, project="cartpole-n_step_QTM")
    wandb.agent(sweep_id, function=main_c_n_step_QTM, count=10_000)

def start_a_n_step_QTM():
    import wandb
    sweep_id = wandb.sweep(sweep=config_acrobot_n_step_QTM, project="acrobot-n_step_QTM")
